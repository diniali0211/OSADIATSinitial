...
from database import save_candidate, get_candidate, update_decision, get_all_candidates
from database import update_decision
...
@app.route("/api/candidates/<candidate_id>/decision", methods=["POST"])
def set_candidate_decision(candidate_id):
    data = request.json or {}

    decision = data.get("decision")
    reason = data.get("reason")

    if decision not in ["approved", "rejected", "kiv"]:
        return jsonify({"error": "Invalid decision"}), 400

    updated = update_decision(candidate_id, decision, reason)
